...
export default function Home() {
  const [works, setWorks] = useState<any[]>([]); // 대표작 상태 추가
  ...
  const fetchData = async () => {
    // 1. 대표작(is_featured: true)만 불러오기
    const { data: worksData, error: worksError } = await supabase
      .from("works")
      .select("*")
      .eq("is_featured", true) // 필터링 조건 추가
    ...
    if (!worksError && worksData) setWorks(worksData);
              ...
              >
                ...
                <div className="mt-8 flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[10px] uppercase tracking-[0.2em] text-ink/50 mt-1">{work.artist} — {work.category}</p>
                  </div>
